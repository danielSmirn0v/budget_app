...
class Budget:
    ...
    @classmethod
    def get_assosiated_budgets(cls,data):
        query = "SELECT users_budgets.*,budget.*, main_bills.* FROM users_budgets LEFT JOIN budget ON users_budgets.budget_id = budget.id LEFT JOIN main_bills ON budget.id = main_bills.budget_main_bills_id WHERE users_budgets.user_id = %(id)s"
